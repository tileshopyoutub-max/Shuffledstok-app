...
export function useAddFile() {
  ...
  const watermarkFileArchive = useArchiveImagesWatermark(
    fileInput.fileType === "archive" ? compressorFileArchive : null,
    settings
  );
  ...
  async function handleSubmit(
    ...
    if (
      fileInput.fileType === "archive" &&
      fileInput.archiveImages.length === 0
    ) {
    ...
    if (!downloadFree) {
      ...
      if (!normalized || !Number.isFinite(value) || value < 0) {
        alert("Please enter a valid price for Premium access");
        return;
      }
    }
