...
interface UserTableProps {
  search: string;
  users: userTypes[];
  onUpdateUser: (id: number, patch: Partial<userTypes>) => void;
}
    ...
    <div className="overflow-x-auto bg-component-dark rounded-xl border border-border-dark">
        ...
        <thead className="border-b border-border-dark">
          <tr className="text-sm text-text-secondary-dark">
            <th className="p-4 font-medium">
              <input
                className="form-checkbox rounded bg-component-dark border-border-dark text-primary focus:ring-primary/50"
                type="checkbox"
              />
            </th>
          ...
          {visibleUsers.map(
            ({ id, avatar, name, email, role, status, dateJoined }) => {
              ...
              return (
                <tr
                  key={id}
                  className="border-b border-border-dark hover:bg-border-dark/50"
                >
                  <td className="p-4">
                    <input
                      className="form-checkbox rounded bg-component-dark border-border-dark text-primary focus:ring-primary/50"
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <p className="font-medium text-text-main-dark">
                          {name}
                        </p>
                  ...
                  <td className="p-4">
                    <div className="flex justify-center gap-2">
                      <button
                        ...
                        className="p-2 text-text-secondary-dark rounded-md hover:bg-border-dark hover:text-text-main-dark"
                      >
      ...
      {editUser && (
        <UserEditModal
          ...
          onSave={(patch) => {
            onUpdateUser(editUser.id, patch);
            setEditUserId(null);
          }}
        />
      )}
