...
export const ViewedImagesSlider = () => {
  ...
  const maxVisible = screen === 'mobile' ? 0 : screen === 'tablet' ? 1 : 2;

  return (
    <div className="relative w-full overflow-hidden flex justify-center items-center" {...handlers}>
      {/* Кнопки снаружи слайдера */}
      {(screen === 'tablet' || screen === 'desktop') && (
        <>
          <button
            onClick={() => {prev(), dispatch(hideHero())}}
            className="absolute left-2 text-white text-4xl z-10 select-none"
          >
            ❮
          </button>
          <button
            onClick={() => {next(), dispatch(hideHero())}}
          ...
          return (
            ...
            >
              <div
                ...
                onClick={() => {dispatch(openImageModal(img)), dispatch(hideHero())}}
              />
            </div>
          );
        })}
