...
export const useFilteredMedia = () => {
  ...
  const { selectedCategories, selectedTags, accessType, photoType, search } = useTypedSelector(
  ...
  const filteredMedia = allMedia.filter(item => {
    ...
    // Фильтр по accessType (только для одиночных изображений)
    if (item.type === "image") {
    ...
    if (selectedCategories.length && !selectedCategories.every(cat => media.categories.includes(cat))) {
      return false
    }

    // Фильтр по тегам
    if (selectedTags.length && !selectedTags.every(tag => media.tags.includes(tag))) {
      return false
    }
